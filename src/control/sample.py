# S/N : XYZARIS0V3P2311N04
# Robot IP : '192.168.1.182'
# script version : v3.2.0.2
...
class RobotMain(object):
    ...
    def __init__(self, robot, **kwargs):
        ...
        self.position_jig_B_grab = [-152.3, -129.0, 192.8, 4.8, 89.0, -90.7]  # linear
        self.position_jig_C_grab = [-78.8, -144.6, 194.4, 4.0, 88.9, -51.8]  # linear
        self.position_sealing_check = [-136.8, 71.5, 307.6, 39.7, -82.8, -28.5]  # Linear
        self.position_capsule_place = [235, 130.8, 461.4, -143.8, 89.4, -57.3]  # Linear
        self.position_before_capsule_place = self.position_capsule_place.copy()
        self.position_before_capsule_place[2] += 25
        self.position_cup_grab = [216.6, -100.2, 147.1, -75.8, -89.6, 146.3]  # linear
        ...
        self.position_jig_B_serve = [201.1, 24.4, 27.2, 109.5, 89.6, 2.1]  # angle
        self.position_jig_C_serve = [-63.1, -138.2, 203, -45.5, 88.1, -112.1]  # Linear
        self.position_capsule_grab = [239.5, 130.0, 463.5, -140.1, 88.0, -52.5]  # Linear
        ...
        while self.connected:
            ...
            try:
                ...
                if self.recv_msg[0] == 'app_ping':
                    ...
                    # 픽업존 아이스크림 뺐는지 여부 확인
                    if self.recv_msg[0].find('icecream_go') >= 0 or self.recv_msg[0].find(
    ...
    def motion_check_sealing(self):
        ...
        self._angle_acc = 200
        try:
            self.clientSocket.send('motion_check_sealing'.encode('utf-8'))
        except:
            pass
    ...
    def motion_grab_cup(self):
        ...
        self._angle_speed = 100
        self._angle_acc = 100
        code = self._arm.set_servo_angle(angle=[-6.5, -20.9, 69.4, 28.0, 30.6, 84.3], speed=self._angle_speed, mvacc=self._angle_acc, wait=False, radius=10.0)
        if not self._check_code(code, 'set_servo_angle'):
            return
        if self.order_msg['makeReq']['cupNum'] in ['A', 'B']:
            print(self.order_msg['makeReq']['cupNum'])
            code = self._arm.set_servo_angle(angle=[-6.5, -22.8, 44.5, 41.2, -22.2, -21.1], speed=self._angle_speed, mvacc=self._angle_acc, wait=False, radius=20.0)
            if not self._check_code(code, 'set_servo_angle'):
                return
            ...
            #    return
            #time.sleep(1)
            code = self._arm.set_position(*[190.0, -96.5, 250.8, -168.0, -87.1, -110.5], speed=self._tcp_speed,
    ...
    def motion_topping(self):
        ...
        if self.order_msg['makeReq']['topping'] == 'bottom':
        ...
        elif self.order_msg['makeReq']['topping'] == 'top':
            ...
            if self.order_msg['makeReq']['jigNum'] in ['A']:
                ...
                if not self._check_code(code, 'set_cgpio_digital'):
                    return
                code = self._arm.set_pause_time(int(self.order_msg['makeReq']['toppingAmount']))
                if not self._check_code(code, 'set_pause_time'):
                    return
                code = self._arm.set_cgpio_digital(0, 0, delay_sec=0)
                if not self._check_code(code, 'set_cgpio_digital'):
            ...
            elif self.order_msg['makeReq']['jigNum'] in ['C']:
                code = self._arm.set_position(*self.position_topping_C_later, speed=self._tcp_speed,
                                              mvacc=self._tcp_acc, radius=0.0, wait=True)
                if not self._check_code(code, 'set_position'):
                    return
        ...
        elif self.order_msg['makeReq']['topping'] == 'none':
    ...
    def motion_serve(self):
        ...
        if self.order_msg['makeReq']['topping'] in ['bottom','none']:
                    ...
                    if self.order_msg['makeReq']['latency'] in ['go']:
                        ...
                        if self.order_msg['makeReq']['sealing'] in ['yes']:
                            ...
                            try:
                                ...
                                print('socket check error : sealing check')
                                pass
                            count = 0
                            while True:
                        ...
                        if self.order_msg['makeReq']['sealing'] in ['go'] or self.order_msg['makeReq']['sealing'] not in ['yes', 'stop']:
                            ...
                            if self.order_msg['makeReq']['topping'] in ['bottom']:
                                self.motion_topping()
                            else :
                                self.motion_topping_pass()
                            self.motion_make_icecream()
                            if self.order_msg['makeReq']['topping'] == 'top':
                                self.motion_topping()
                            self.motion_serve()
...
if __name__ == '__main__':
    ...
    arm = XArmAPI('192.168.1.182', baud_checkset=False)
